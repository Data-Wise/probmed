...
- $`Y_{x^*, M_x}`$: Outcome under control ($`x^*`$) with mediator at its
  treated level ($`M_x`$)
- $`Y_{x, M_x}`$: Outcome under treatment ($`x`$) with mediator at its
  treated level ($`M_x`$)
...
When exposure-mediator interactions are present ($`X \times M`$ in
outcome model):
...
$`Y = \beta_0 + \beta_1 X + \beta_2 M + \beta_3 C + \epsilon_Y`$:
...
When
$`Y = \beta_0 + \beta_1 X + \beta_2 M + \beta_3 XM + \beta_4 C + \epsilon`$:

``` math
P_{\text{med}}(x_0) = P(Y_{x^*, M_x} > Y_{x, M_x} | X = x_0)
```

Conditioning value $`x_0`$ must be specified; P_med varies by treatment
level.
