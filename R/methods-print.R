...
S7::method(print, PmedResult) <- function(x, digits = 3, ...) {
  ...
  if (!is.na(x@ci_lower)) {
    cat(
      paste0(x@ci_level * 100, "% CI: ["),
      format(x@ci_lower, digits = digits), ", ",
      format(x@ci_upper, digits = digits), "]\n"
    )
  }

  if (!is.na(x@ie_estimate)) {
    cat("\nIndirect Effect (Product of Coefficients):\n")
    cat("Estimate:", format(x@ie_estimate, digits = digits), "\n")
    if (!is.na(x@ie_ci_lower)) {
      cat(
        paste0(x@ci_level * 100, "% CI: ["),
        format(x@ie_ci_lower, digits = digits), ", ",
        format(x@ie_ci_upper, digits = digits), "]\n"
      )
    }
  }
